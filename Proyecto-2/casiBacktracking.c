 ...
 */
    ...
    if (positionX - 1 >= 0 && matrix[positionX - 1][positionY] == 0)
    { // Up
...
void searchRandomFrontierCell(int *positionX, int *positionY, int *frontierCellsXPosition, int *frontierCellsYPosition, int *frontierCellsArraySize)
{ // Searchs a random Frontier Cell
    /*
...
void searchRandomSpanningTreeCell(int **matrix, int rows, int columns, int positionX, int positionY, int *randomSpanningTreeCellX, int *randomSpanningTreeCellY)
{ // Search a random Spanning Tree Cell adjacent to a Frontier Cell
    /*
...
void removeBarrierCell(int **matrix, int frontierX, int frontierY, int spanningTreeCellX, int spanningTreeCellY)
{ // Removes a barrier cell between two coordinates
    /*
...
void stepTwoAndThree(int **matrix, int rows, int columns, int *frontierCellsXPosition, int *frontierCellsYPosition, int *frotierCellsArraySize)
{ // Step Two and Three of the algorithm
    /*
...
void newRandomPosition(int **matrix, int rows, int columns, int currentPositionX, int currentPositionY, int *newPositionX, int *newPositionY)
{
    ...
    while (flag)
    {
        ...
        case 0: // el % 16 es para asegurarse que no se salga del rango de indices de la tabla
            if (currentPositionX - 1 >= 0 && canGoUp(matrix[currentPositionX][currentPositionY] % 16))
            { // Up
                *newPositionX -= 1;
    ...
    while ((currentPositionX != laberynthExitX) || (currentPositionY != laberynthExitY))
    {
        ...
        if (currentPositionX - 1 > 0 && canGoUpp)
        {
            ...
            if (valueUp < 16)
            {
                ...
                // Save position
                positions[positionCount][0] = newPositionX;
                positions[positionCount][1] = currentPositionY;
                positionCount++;
        ...
        else
        {
            ...
            positionCount--;
            currentPositionX = positions[positionCount][0];
            currentPositionY = positions[positionCount][1];
